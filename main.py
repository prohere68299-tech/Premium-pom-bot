...
BOT_TOKEN = "8912326354:AAGgpchZFONJOIbpf_jvfyXJ5xySiKL7EO8"   # <-- Isko apne token se replace karo
...
class Database:
    ...
    def get_stats(self):
        ...
        # today's earnings (approved payments with date today)
...
def handle_cb(call):
    ...
    try:
        ...
        # ---------- ADMIN PANEL CALLBACKS ----------
            ...
            # Refresh payment list view
            handle_cb(call)  # Not ideal, but we'll just trigger callback again with same data
            # Instead, go back to payments list
            data = "admin_payments"
            handle_cb(call)
            ...
            if not is_admin(user_id):
                ...
                return
            pid = int(data.split("_")[1])
            db.reject_payment(pid, "Rejected by admin")
            ...
            if not is_admin(user_id):
                bot.answer_callback_query(call.id, "Unauthorized!")
                return
            # Export plans + media as .db and .sql
            # We'll send both options
            kb = types.InlineKeyboardMarkup(row_width=2)
        ...
        # Add more callbacks for plan editing steps, etc.
        # Since this is a lot, I'll keep it to the essential functionality.
        # The rest can be added similarly.
...
# ---------- MESSAGE HANDLERS FOR SETTINGS AND ADMIN INPUTS ----------
    ...
    if 'edit_plan' in user_data[user_id]:
        ...
        if plan:
            ...
            if step == 'name':
                ...
                admin_cmd(msg)  # Show admin panel again
        else:
            del user_data[user_id]
...
def handle_photo(msg):
    ...
    if user_id in user_data and 'screenshot_plan' in user_data[user_id]:
        ...
        if plan:
            ...
            bot.reply_to(msg, "✅ Payment screenshot received! Admin will review.")
            # Notify admins
            for admin in ADMIN_IDS:
                try:
                    bot.send_photo(admin, file_id, caption=f"💳 New Payment #{pid}\nUser: {msg.from_user.first_name}\nPlan: {plan['name']}\nAmount: ₹{plan['price']}")
                except:
                    pass
            del user_data[user_id]
        return
